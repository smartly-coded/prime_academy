...
class _ReorderQuestionDialogState extends State<ReorderQuestionDialog> {
  ...
  void _placeAnswer(int position) {
    if (_selectedAnswerIndex != null) {
      setState(() {
        // إزالة الكلمة من موضعها السابق إذا كانت موجودة
        _orderedAnswers.replaceRange(
          0,
          _orderedAnswers.length,
          _orderedAnswers
              .map(
                (answer) =>
                    answer?.id == _shuffledAnswers[_selectedAnswerIndex!].id
                    ? null
                    : answer,
              )
              .toList(),
      ...
      child: Column(
        children: [
          ...
          Wrap(
            ...
            children: List.generate(_orderedAnswers.length, (index) {
