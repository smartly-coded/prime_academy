...
//   //   }
//   // }
// // field جديدة - حفظ فيها الكافأة بدأت من أي سؤال
// bool _lessonRewardClaimed = false;

// // حدّل _changeVideo - شيل الـ condition عشان يـ clear دايما
// void _changeVideo(String url) {
//     if (_isDisposing || !mounted) return;
...
//         _isPlayerReady = false;
//         _shownQuestions.clear();
//         _firstQuestionAnswered = false;
//         _lessonRewardClaimed = false; // reset الكافأة
...
//     // بس skip لو كان الدرس rewarded من قبل (قبل ما يدخل الـ view)
//     // أما لو _firstQuestionAnswered خليها تفضل تظهر باقي الأسئلة
//     if (_rewardedLessons.contains(_currentSelectedItemId) && !_firstQuestionAnswered) {
//       print(
//         'Lesson $_currentSelectedItemId was already rewarded before - skipping questions',
//       );
//       return;
//     }
...
//   //   }
//   // }
//  // حدّل _checkForQuestionsAtPosition كده كله
// void _checkForQuestionsAtPosition(Duration position) {
//     if (_lessonQuestions.isEmpty) return;

//     // بس skip لو الدرس كان rewarded قبل ما يدخل الـ view وقبل ما يجاوب أي سؤال
//     if (_rewardedLessons.contains(_currentSelectedItemId) && !_firstQuestionAnswered) {
//       return;
//     }
...
//     // ✅ جمع كل الأسئلة اللي لازم تظهر دلوقتى
//     // يعني كل سؤال timestamp أقل من أو يساوي currentSeconds ومش اتجاب بعد
...
//       // ✅ لو السؤال فاتنا (timestamp أقل من currentSeconds) ومش اتجاب يضيفه
//       if (currentSeconds >= question.timestamp && !alreadyAnswered) {
//         questionsToShow.add(question);
//       }
//     }

//     if (questionsToShow.isEmpty) return;

//     // pause الفيديو قبل ما نعرض أي سؤال
//     _playerKey?.currentState?.pause();

//     // ✅ حفظ كل الأسئلة اللي هنعرضها في قائمة عشان نـ navigate لهم واحد واحد
//     _pendingQuestions = questionsToShow;
//     _showNextPendingQuestion();
//   }
...
//       // كل الأسئلة اتعرضت، كمّل الفيديو
//       _resumeVideo();
//       return;
//     }
...
//           final questionKey = '${_currentSelectedItemId}_${_currentAnsweredQuestionId}';
//           _shownQuestionsGlobally.add(questionKey);
//           _saveShownQuestions();
...
//       },
//       error: (error) {
//         // ✅ لو حصل خطأ متحفظش السؤال عشان يظهر تاني
//         _currentAnsweredQuestionId = null;
//         _showErrorDialog("خطأ في إرسال الإجابة: $error");
//       },
//       loading: () {},
  ...
  void _changeVideo(String url) {
    ...
    if (videoId != null) {
      setState(() {
        ...
        // ✅ load الأسئلة المتجاوبة من الـ SharedPreferences
        _shownQuestions.clear();

        for (var key in _shownQuestionsGlobally) {
          if (key.startsWith('${_currentSelectedItemId}_')) {
            final id = int.tryParse(key.split('_').last);
            if (id != null) _shownQuestions.add(id);
          }
        }
